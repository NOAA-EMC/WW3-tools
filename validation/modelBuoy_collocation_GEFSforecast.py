...
"""
modelBuoy_collocation_GEFSforecast.py

VERSION AND LAST UPDATE:
 v1.0  04/04/2022

PURPOSE:
 Collocation/pairing GEFS ww3 point output ensemble forecasts 
  with wave buoys.
 Matchups of ww3 results and buoy data are generated, for the same
  points (lat/lon) and time.

USAGE:
 This code is designed for ww3 ensemble forecasts, with three dimensions 
  per point (ensemble member, cycle time, and forecast lead time).
 Ww3 netcdf results for point output tables (tab) and spectral output
  is utilized. Only spec point output has wind speed.
 ...
 The ensemble members (and its name directories) are saved 
  in enslist.txt, which can be informed or leave os.system take it.
 ...
 It also reads the grid mask, to exclude land points and/or nearcoast
  points as defined in preprGridMask.py. This mask also inform ocean 
  names, and Offshore Marine Zones NOAA/OPC forecast areas.
 The cyclone IDs (cyclonic region or not) is another input, generated
  with procyclmap.py
 Edit mww3p, ndbcp, and copernp to set wave data paths and names.
 Edit gmask, and cyclonep, for grid mask and cyclone maps.
 Python code can be run directly, without input arguments.

OUTPUT:
 netcdf file GEFSww3buoy_collocation*.nc containing the matchups of buoy 
  and ww3 data, for the stations (lat/lon) where both data sources 
  are available; containing ensemble members, time(cycle), and forecast
  lead time.
  Information of cyclone, water depth, distance to coast, ocean names, 
  and Offshore Marine Zones NOAA/OPC forecast areas.

DEPENDENCIES:
 See dependencies.py and the imports below.
 ww3 table results in netcdf format (list of files ww3list.txt)
 ww3 spectral results in netcdf format.
 NDBC buoy data (see retrieve_ndbc_nc.py)
 Copernicus buoy data (see get_buoydata_copernicus.sh)
 CycloneMap_*.nc files generated by procyclmap.py
 Grid mask gridInfo_GEFS.nc generated by preprGridMask.py
...
vbwnd = ncfile.createVariable('obs_wsp',np.dtype('float32').char,('buoypoints','fcycletime','forecastleadtime'))
