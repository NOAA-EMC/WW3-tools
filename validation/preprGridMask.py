# Create a grid mask to identify coastal points and open/deep water points, based on water depth and distance to the coast.
# this is useful for model validation against satellite date, where coastal areas should be excluded, as well as to run specific assessments comparing deep water with coastal areas.
# the grid mask uses the same resolution (lat/lon) array as given sample file (see xr.open_dataset below).
# the file distFromCoast.nc was generated by organizeDistanceToCoast.py script.
# you can replace etopo1.nc bathymetry by any other source, such as GEBCO. Just pay attention to longitude standards (-180to180 or 0to360).
# Shapefiles for the Ocean Names and forecast areas must be downloaded, as indicated below. And stored in a proper directory with path informed below.
...
print(' allocating ocean names. This may take a while ...')
...
print(' allocating NWS Forecast areas. This may take a while ...')
