...
Nsubsample=10# ~1. hours
Nsubsample=20# ~1. hours
...
#Average output to resolution of DistToCoast
# load topo file to build distance function on the grid of
data = nc.Dataset("../RWPS/Data/RTopo_2_0_4_GEBCO_v2023_60sec_pixel.nc","r")
xlon = np.asarray(data["lon"][:])
ylat = np.asarray(data["lat"][:])
...
#eW = signal.convolve2d(elev,W, boundary='symm', mode='same')
eW = signal.convolve2d(elev,W, mode='same')

xmid = 0.5 * (xlon[:-1:] + xlon[1::])
ymid = 0.5 * (ylat[:-1:] + ylat[1::])
xmid=xmid[0 :: Nsubsample]
ymid=ymid[0 :: Nsubsample]
eW=eW[0 :: Nsubsample,0 :: Nsubsample]
topofl="TopographyFor"+flout
#output to jigsaw .msh format
topo = jigsawpy.jigsaw_msh_t() 
topo.mshID = "ellipsoid-grid"
topo.radii = np.full( +3, +6371.0, dtype=jigsawpy.jigsaw_msh_t.REALS_t)
topo.xgrid = xmid
topo.ygrid = ymid 
topo.value = eW
jigsawpy.savemsh(topofl, topo)
