...
def timeseries(*args):
	...
	if (np.size(model.shape)>2) | (np.size(obs.shape)>2):
	...
	if np.size(mlabels)>0 and np.size(np.where(obs[0,:]>-999))>1:
	...
	plt.savefig(ftag+'TimeSeries.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
...
def qqplot(*args):
	...
	for i in np.array([50,80,90,95,99]):
		plt.axvline(x= np.nanpercentile(obs,int(i)),ls='--',color='grey',linewidth=1.,alpha=0.9,zorder=1)
		plt.text(np.nanpercentile(obs,int(i)),(aux.max()-aux.min())/15 + aux.min(),str(int(i))+'th',color='dimgrey',fontsize=sl-7,zorder=4)
		plt.text(np.nanpercentile(obs,int(i)),(aux.max()-aux.min())/1.05 + aux.min(),str(int(i))+'th',color='dimgrey',fontsize=sl-7)
	...
	plt.tight_layout(); #plt.axis('tight') 
	plt.savefig(ftag+'QQplot.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	plt.close(fig1); del fig1, ax
...
def scatterplot(*args):
	...
	if (model.shape[0]>1) & (np.size(mlabels)==0):
	...
	if obs[0,:].shape[0]>50000:
		sk=int(np.round(np.float(obs[0,:].shape[0])/30000.,0))
	else:
		sk=1
	...
	for i in np.array([50,80,95,99]):
		plt.axvline(x= np.nanpercentile(obs,int(i)),ls='--',color='grey',linewidth=1.,alpha=0.7,zorder=1)
		plt.text(np.nanpercentile(obs,int(i)),((famax-famin)/15)+famin,str(int(i))+'th',color='dimgrey',fontsize=sl-7,zorder=4)
		plt.text(np.nanpercentile(obs,int(i)),((famax-famin)/1.05)+famin,str(int(i))+'th',color='dimgrey',fontsize=sl-7,zorder=4)
	...
	plt.tight_layout(); #plt.axis('tight')
	plt.savefig(ftag+'ScatterPlot.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	plt.close(fig1); del fig1, ax
...
def taylordiagram(*args):
	...
	plt.tight_layout()
	plt.savefig(ftag+'TaylorDiagram.png', dpi=250, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	plt.close(fig1); del fig1, ax
...
def combinerrors(*args):
	...
	plt.tight_layout(); #plt.axis('tight') 
	plt.savefig(ftag+'NBiasSI.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	plt.close(fig1); del fig1, ax
...
def pdf(*args):
	...
	plt.savefig(ftag+'PDF.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	ax.set_yscale( "log" ); plt.axis('tight');plt.ylim(ymin = 0.001);plt.ylim(ymax = 1.)
	plt.savefig(ftag+'PDFlogscale.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	plt.close(fig1); del fig1, ax
...
def errXftime(*args):
	...
	if (np.size(model.shape)>2) | (np.size(ftime.shape)>1):
	...
	plt.tight_layout(); # plt.axis('tight') 
	plt.grid(c='grey', ls='--', alpha=0.3,zorder=1)
	plt.savefig(ftag+'ErrXFTime.png', dpi=200, facecolor='w', edgecolor='w',orientation='portrait', format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
	plt.close(fig1); del fig1, ax
