...
def prepare_and_interpolate(model_data, variable_name):
    ...
    # Satellite data points for interpolation
    satellite_points = np.array([
        satellite_data['latitude'].values,
        satellite_longitudes,
        satellite_times_unix
    ]).T

    # Perform interpolation
    interpolated_values = interpolator(satellite_points)
...
# Create DataArrays for satellite data and interpolated model data
...
# Create DataArrays for longitude and latitude with units
...
# Add global attribute with explanation about the time units
interpolated_dataset.attrs['global_attribute'] = 'Time units are seconds since 1970-01-01 00:00:00'

# Save the combined dataset to a NetCDF file
interpolated_dataset.to_netcdf(output_file)
