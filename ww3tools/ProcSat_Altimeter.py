...
PURPOSE:
 ...
 (1) It can collocate tracks into a regular lat/lon grid (a ww3 sample output
...
def along_track(AODN,atime,wconfig):
    ...
    Output: pandas dataframe containing: TIME (seconds since 1970), SAT_TIME (seconds since 1970), LATITUDE, LONGITUDE,
      WDEPTH, DISTCOAST, HS, HS_CAL, WSPD, WSPD_CAL
...
# the old precessing, collocation to model grid, from gridSatGlobal_Altimeter, will enter here
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 6 :
        ww3grid=str(sys.argv[5]) # ww3 field output sample (file name) to take lat lon arrays, or any gridinfo.nc with lat lon arrays inside.
    ...
    # elif wconfig['tspace']==1:
        # copy from gridSatGlobal_Altimeter.py
        # Function gridded data using model grid points.
