...
def process_satellite_data(dirs, sdname, sname, start_date, end_date, enable_time_averaging=True):
...
if __name__ == '__main__':
    ...
    # Process the data for the specified satellite with time averaging enabled
    time_averaged_data = process_satellite_data(dirs, sdname, sname, start_date, end_date, enable_time_averaging=True)
    ...
    # Process the data for the specified satellite with time averaging disabled
    raw_data = process_satellite_data(dirs, sdname, sname, start_date, end_date, enable_time_averaging=False)

    if raw_data is not None:
        # Save the raw data in a new NetCDF file
        raw_output_file = f"{sdname}_raw_data.nc"
        with nc.Dataset(raw_output_file, 'w', format='NETCDF4') as ncfile:
            # Define dimensions
            ncfile.createDimension('TIME', None)
            ...
            # Fill variables with raw data
            time_var[:] = raw_data['TIME']
            lat_var[:] = raw_data['LATITUDE']
            lon_var[:] = raw_data['LONGITUDE']
            swh_ku_var[:] = raw_data['SWH_KU']
            swh_ku_cal_var[:] = raw_data['SWH_KU_CAL']
            wspd_var[:] = raw_data['WSPD']
            wspd_cal_var[:] = raw_data['WSPD_CAL']
            sig0_ku_std_dev_var[:] = raw_data['SIG0_KU_std_dev']
            swh_ku_num_obs_var[:] = raw_data['SWH_KU_num_obs']
            swh_ku_std_dev_var[:] = raw_data['SWH_KU_std_dev']
            swh_ku_qc_var[:] = raw_data['SWH_KU_quality_control']

        print(f"Raw data for {sname} saved in {raw_output_file}")
