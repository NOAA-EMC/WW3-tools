...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collocate WW3 point output with buoy observations."
    )

    parser.add_argument(
        "ww3_list",
        help="Text file containing WW3 point-output file paths."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Forecast data structure flag. 0 = continuous/hindcast; >0 = forecast cycle/lead-time structure."
    )

    parser.add_argument(
        "-g", "--gridinfo",
        default="",
        help="Optional gridInfo NetCDF file generated by prepGridMask.py."
    )

    parser.add_argument(
        "-c", "--cyclonemap",
        default="",
        help="Optional CycloneMap NetCDF file generated by procyclmap.py. Requires --gridinfo."
    )

    parser.add_argument(
        "--ndbcp",
        default="/work/noaa/marine/ricardo.campos/data/buoys/NDBC/ncformat/wparam",
        help="Directory containing NDBC buoy wave-parameter NetCDF files."
    )

    parser.add_argument(
        "--copernp",
        default="/work/noaa/marine/ricardo.campos/data/buoys/Copernicus/wtimeseries",
        help="Directory containing Copernicus buoy time-series NetCDF files."
    )
...
wlist = np.atleast_1d(np.loadtxt(args.ww3_list, dtype=str))
ftag = str(args.ww3_list).split('list')[1].split('.txt')[0]

forecastds = args.forecastds

gridinfo = args.gridinfo if args.gridinfo else 0
cyclonemap = args.cyclonemap if args.cyclonemap else 0

ndbcp = args.ndbcp
copernp = args.copernp

# basic check
print(" Reading ww3 list " + str(args.ww3_list))
print(" Tag " + str(ftag))
...
if gridinfo != 0:
    print(" Using gridInfo " + str(gridinfo))

if cyclonemap != 0 and gridinfo == 0:
    sys.exit("ERROR: --cyclonemap requires --gridinfo.")

if cyclonemap != 0:
    print(" Using cyclone map " + str(cyclonemap))

print(" NDBC path       : " + str(ndbcp))
print(" Copernicus path : " + str(copernp))
