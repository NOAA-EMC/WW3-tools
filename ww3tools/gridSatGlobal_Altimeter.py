...
ahsk=np.zeros((10**pia),'f'); ahskcal=np.zeros((10**pia),'f')
...
if np.size(indq)>10:
	...
	ahsk=np.copy(ahsk[indq[0]]); ahskcal=np.copy(ahskcal[indq[0]])
	...
	fhskcal=np.zeros((ast.shape[0]*2),'f'); fwnd=np.zeros((ast.shape[0]*2),'f'); fwndcal=np.zeros((ast.shape[0]*2),'f')
	...
	del ast, aslat, aslon, ahsk, ahskcal, awnd, awndcal, indq, atime
	...
	del vft, vflat, vflon, vfhsk, vstdhsk, vcounthsk, vfhskcal, vfwnd, vfwndcal
