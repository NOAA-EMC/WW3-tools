...
//      if (isdigit(name[0])) strcpy(tmpname, "N");  // no names starting with digit
      if (! isalpha(name[0])) strcpy(tmpname, "N");  // names must start with a-z or A-Z
      strcat(tmpname, name);
	...
	if (isalnum(tmpname[i])) { varname_buf[j++] = tmpname[i++] ; continue; }
	varname_buf[j++] = '_'; i++; continue;
      }
      varname_buf[j] = 0;
    }
