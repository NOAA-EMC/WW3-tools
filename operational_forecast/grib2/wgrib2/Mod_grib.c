...
int f_set_date(ARG1) {
    ...
    if (check_date(year, month, day) != 0 || hour < 0 || hour >= 24 ||
...
static const char *set_options="discipline, center, subcenter, master_table, local_table, background_process_id, "
        "analysis_or_forecast_process_id, table_1.2, table_1.3, table_1.4, table_3.0, table_3.1/GDT" 
...
int f_set(ARG2) {
	...
	if (strcmp(arg1,"background_process_id") == 0) {
	    *background_generating_process_identifier_location(sec) = (unsigned char) i;
	    return 0;
	}
	if (strcmp(arg1,"analysis_or_forecast_process_id") == 0) {
	    *analysis_or_forecast_generating_process_identifier_location(sec) = (unsigned char) i;
	    return 0;
	}
