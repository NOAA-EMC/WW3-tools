...
# Script to fetch NCEP Ensemble forecasts GFS
# -------------------------------------------
...
VARIABLES=":UGRD:850 mb:|:VGRD:850 mb:|:GUST:surface:|:HGT:850 mb:|:RH:2 m above ground:|:PRMSL:mean sea level:"
...
for forecast_hour in $_forecast_hours ; do
	...
	url="${SERVER}/gfs.${DATE}/${HOUR}/atmos/gfs.t${HOUR}z.pgrb2.0p25.f${forecast_hour}"
	dpath="${_DOWNCAST_CACHE_DIR}/gfs.pgrb2.0p25.${DATE}${HOUR}.f${forecast_hour}"
...
file_list=${_DOWNCAST_CACHE_DIR}/gfs.pgrb2.0p25.${DATE}${HOUR}.f*.nc
downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
filename="gfs.pgrb2.0p25.${DATE}${HOUR}.nc"
