...
for ensemble in $_ensembles ; do
	...
	for forecast_hour in $_forecast_hours; do
		...
		url="${SERVER}/gefs.${DATE}/${HOUR}/pgrb2ap5_bc/ge${ensemble}.t${HOUR}z.pgrb2a.0p50_bcf${forecast_hour}"
		dpath="${_DOWNCAST_CACHE_DIR}/${PREFIX}.${ensemble}.pgrb2a.0p50_bc.f${forecast_hour}"
	...
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/${PREFIX}.${ensemble}.pgrb2a.0p50_bc.*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="gefs.${ensemble}.${DATE}${HOUR}.pgrb2a.0p50_bc.nc"
