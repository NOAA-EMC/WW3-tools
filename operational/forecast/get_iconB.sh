...
VARIABLES_1='pmsl vmax_10m t_2m relhum_2m'
VARIABLES_2='fi u v'
ATM_LEVEL='850' # atm level for VARIABLES_2
...
for var in $VARIABLES_1; do
	nfiles=0
	for forecast_hour in $_forecast_hours ; do
		[[ $var == 'vmax_10m' ]] && [[ $forecast_hour == '000' ]] && continue # skip zero hour for vmax_10m
		...
		dpath="${_DOWNCAST_CACHE_DIR}/icon.${DATE}${HOUR}.${var^^}.f${forecast_hour}"
		download -wget "$url" "$dpath" &&
			post_process -cdo "$dpath" &
		nfiles=$(($nfiles+1))
	done
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/icon.${DATE}${HOUR}.${var^^}.f*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="icon.${DATE}${HOUR}.${var^^}.nc"
...
for var in $VARIABLES_2; do
	nfiles=0
	for forecast_hour in $_forecast_hours ; do
		[[ $(jobs -p|wc -l) -gt $((${NJOBS}-1)) ]] && wait -n # manage running jobs
		url="${SERVER}/${HOUR}/${var}/icon_global_icosahedral_pressure-level_${DATE}${HOUR}_${forecast_hour}_${ATM_LEVEL}_${var^^}.grib2"
		dpath="${_DOWNCAST_CACHE_DIR}/icon.${DATE}${HOUR}.${var^^}.f${forecast_hour}"
		download -wget "$url" "$dpath" &&
			post_process -cdo "$dpath" &
		nfiles=$(($nfiles+1))
