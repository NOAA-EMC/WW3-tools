...
# Script to fetch NCEP Ensemble forecasts CMCE
# --------------------------------------------
...
for ensemble in $_ensembles ; do
	...
	for forecast_hour in $_forecast_hours; do
		...
		url="${SERVER}/cmce.${DATE}/${HOUR}/pgrb2ap5/cmc_ge${ensemble}.t${HOUR}z.pgrb2a.0p50.f${forecast_hour}"
		dpath="${_DOWNCAST_CACHE_DIR}/cmce_ge${ensemble}.${DATE}${HOUR}.pgrb2a.0p50.f${forecast_hour}"
	...
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/cmce_ge${ensemble}.${DATE}${HOUR}.pgrb2a.0p50.*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="cmce.${ensemble}.${DATE}${HOUR}.pgrb2a.0p50.nc"
