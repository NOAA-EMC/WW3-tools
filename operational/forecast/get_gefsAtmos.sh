...
VARIABLES=":UGRD:850 mb:|:UGRD:10 m above ground:|:VGRD:850 mb:|:VGRD:10 m above ground:|:PRMSL:mean sea level:|:HGT:850 mb:|:TMP:2 m above ground:|:RH:2 m above ground:"
VARIABLES_GUST=":GUST:surface:"
...
_forecast_hours="$(seq -f '%03g' 0 3 240) $(seq -f '%03g' 246 6 840)"
_forecast_hours_gust="$(seq -f '%03g' 0 3 240)"
...
for ensemble in $_ensembles ; do
	...
	for forecast_hour in $_forecast_hours; do
		...
		url="${SERVER}/gefs.${DATE}/${HOUR}/atmos/pgrb2ap5/ge${ensemble}.t${HOUR}z.pgrb2a.0p50.f${forecast_hour}"
		dpath="${_DOWNCAST_CACHE_DIR}/gefs.${ensemble}.${DATE}${HOUR}.pgrb2a.0p50.f${forecast_hour}"
		download -get-perl "$url" "$dpath" "$VARIABLES" &&
			post_process -wgrib "$dpath" &
		nfiles=$((nfiles+1))
	done
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/gefs.${ensemble}.${DATE}${HOUR}.pgrb2a.0p50.f*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="gefs.${ensemble}.${DATE}${HOUR}.pgrb2a.0p50.nc"
	...
	# GUST
	nfiles=0
	for forecast_hour in $_forecast_hours_gust; do
		[[ $(jobs -p|wc -l) -gt $((${NJOBS}-1)) ]] && wait -n # manage running jobs
		url="${SERVER}/gefs.${DATE}/${HOUR}/atmos/pgrb2sp25/ge${ensemble}.t${HOUR}z.pgrb2s.0p25.f${forecast_hour}"
		dpath="${_DOWNCAST_CACHE_DIR}/gefs_gust.${ensemble}.${DATE}${HOUR}.pgrb2s.0p25.f${forecast_hour}"
		download -get-perl "$url" "$dpath" "$VARIABLES_GUST" &&
			post_process -wgrib "$dpath" &
		nfiles=$((nfiles+1))
	done
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/gefs_gust.${ensemble}.${DATE}${HOUR}.pgrb2s.0p25.f*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="gefs_gust.${ensemble}.${DATE}${HOUR}.pgrb2s.0p25.nc"
	# verify if all files were downloaded, if so concatenate them
	if [[ $nfiles -eq $downloaded_files ]] ; then
		concatenate_records ${_DOWNCAST_CACHE_DIR}/${filename} -- $file_list &&
			rename_variables ${_DOWNCAST_CACHE_DIR}/${filename} ${_RENAME_CONF_FILE} &&
