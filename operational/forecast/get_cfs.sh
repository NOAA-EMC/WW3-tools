...
# Script to fetch NCEP Ensemble forecasts CFS
# -------------------------------------------
...
DATE=${DATE:=$(date -u --date='-1 day' '+%Y%m%d')}
HOUR=${HOUR:="00"}
...
SERVER="https://www.ftp.ncep.noaa.gov/data/nccf/com/cfs/prod/cfs"

STATIONS=("pgbf" "flxf" "ocnf")
declare -A VARIABLES=()
VARIABLES[${STATIONS[0]}]=":UGRD:850 mb:|:VGRD:850 mb:|:HGT:850 mb:|:RH:2 m above ground:|:PRMSL:mean sea level:"
VARIABLES[${STATIONS[1]}]=":UGRD:10 m above ground:|:VGRD:10 m above ground:|:TMP:2 m above ground:"
VARIABLES[${STATIONS[2]}]=":TMP:surface:"
...
_forecast_hours="$(seq -f '%03g' 0 6 720)"
_server_id='01' # NOAA server id

for st in ${STATIONS[@]} ; do
	nfiles=0
	for forecast_hour in $_forecast_hours ; do
		[[ $st == 'ocnf' ]] && [[ $forecast_hour == '000' ]] && continue # skip hour zero for ocnf

		[[ $(jobs -p|wc -l) -gt $((${NJOBS}-1)) ]] && wait -n # manage running jobs
		fc_date=$(date -ud "$DATE +$forecast_hour hour" '+%Y%m%d%H')
		url="${SERVER}/cfs.${DATE}/${HOUR}/6hrly_grib_${_server_id}/${st}${fc_date}.${_server_id}.${DATE}${HOUR}.grb2"
		dpath="${_DOWNCAST_CACHE_DIR}/${PREFIX}.${st}.${_server_id}.f${forecast_hour}"
		download -get-perl "$url" "$dpath" "${VARIABLES[$st]}" &&
			post_process -wgrib "$dpath" &
		nfiles=$(($nfiles+1))
	done

	wait
	file_list=${_DOWNCAST_CACHE_DIR}/${PREFIX}.${st}.${_server_id}.f*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="${PREFIX}.${st}.${DATE}${HOUR}.nc"
	...
	if [[ "$nfiles" -eq "$downloaded_files" ]] ; then
		...
		log_error "$LOG_FILE" "Failed to download/process some files from $st"
	fi
done
