...
# Script to fetch NCEP Ensemble forecasts FENS
# --------------------------------------------
...
SERVER="https://www.ftp.ncep.noaa.gov/data/nccf/com/naefs/prod"
...
_ensembles=$(seq -f '%03g' 1 1 20)
_forecast_hours="$(seq -f '%03g' 0 6 384)"

for ensemble in $_ensembles ; do
	nfiles=0
	for forecast_hour in $_forecast_hours; do
		[[ $(jobs -p|wc -l) -gt $((${NJOBS}-1)) ]] && wait -n # manage running jobs
		url="${SERVER}/fens.${DATE}/${HOUR}/pgrb2ap5/ENSEMBLE.halfDegree.MET.fcst_et${ensemble}.${forecast_hour}.${DATE}${HOUR}"
		dpath="${_DOWNCAST_CACHE_DIR}/fens.${ensemble}.${DATE}${HOUR}.pgrb2a.f${forecast_hour}"
	...
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/fens.${ensemble}.${DATE}${HOUR}.pgrb2a.f*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="fens.${ensemble}.${DATE}${HOUR}.pgrb2a.nc"
	# verify if all files were downloaded, if so concatenate them
	if [[ $nfiles -eq $downloaded_files ]] ; then
		concatenate_records ${_OUT_DIR}/${filename} -- $file_list &&
			rename_variables ${_OUT_DIR}/${filename} ${_RENAME_CONF_FILE} &
	else
