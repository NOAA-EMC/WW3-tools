...
# Script to fetch Wave ICON forecast data
# -----------------------------------------------
...
SERVER="https://opendata.dwd.de/weather/maritime/wave_models/gwam/grib"

VARIABLES="mdts mdww mpts mpww mwd ppts ppww shts shww swh tm10"
...
_forecast_hours="$(seq -f '%03g' 0 3 174)"

for var in $VARIABLES; do
	nfiles=0
	for forecast_hour in $_forecast_hours ; do
		[[ $(jobs -p|wc -l) -gt $((${NJOBS}-1)) ]] && wait -n # manage running jobs
		url="${SERVER}/${HOUR}/${var}/GWAM_${var^^}_${DATE}${HOUR}_${forecast_hour}.grib2"
		dpath="${_DOWNCAST_CACHE_DIR}/wamicon.${DATE}${HOUR}.${var^^}.f${forecast_hour}"
	...
	wait
	file_list=${_DOWNCAST_CACHE_DIR}/wamicon.${DATE}${HOUR}.${var^^}.f*.nc
	downloaded_files=$(echo $file_list | tr ' ' '\n' | wc -l)
	filename="wamicon.${DATE}${HOUR}.${var^^}.nc"
