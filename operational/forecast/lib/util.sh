...
[[ -x ${BUNZIP:=$(which bunzip2)} ]]    || { log_error "$LOG_FILE" "bunzip2 does not exist. A custom path can be given to the program with the BUNZIP variable." ; exit 1 ;}
...
# Helper function to decompress a file, if no decompression is needed nothing happens
# ---------------
## Arguments
#  $1 -- Filename withouth extension
#  $2 -- Log file
_decompress() {
	# decompress if needed, FIXME this might fail if file is corrupted
	if [[ -f "${1}.grb2.bz2" ]] ; then
		$BUNZIP -f "${1}.grb2.bz2" &>>$2
	fi
}

# Helper function to convert a file into NETCDF format
# ---------------
## Arguments
#  $1 -- Program to use on convertion (-wgrib or -cdo). *NOTE*: -cdo needs TARGET_FILE and WEIGHTS_FILE defined
#  $2 -- Filename withouth extension
#  $3 -- Log file
_convert_to_nc() {
	case "$1" in
		-wgrib)
			$WGRIB2 "${2}.grb2" -netcdf "${2}.nc" &>>$3
			# $CDO -f nc copy "${2}.grb2" "${2}.nc" &>>$3
			;;
		-cdo)
			$CDO -f nc remap,${TARGET_FILE},${WEIGHTS_FILE} "${2}.grb2" "${2}.nc" &>>$3
			;;
		*) # unknown option, throw error and stop script
			log_error "$3" "Unknown process mode given to function ($1). Must be -wgrib or -cdo"
