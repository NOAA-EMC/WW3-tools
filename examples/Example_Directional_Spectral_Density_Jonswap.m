...
t1=(datenum('20180701 000000','yyyymmdd HHMMSS')); % first timestep 
t2=(datenum('20180702 000000','yyyymmdd HHMMSS')); % last timestep 
dt=1;                      %delta t (day)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
DeltaDir=360/nDir;
time=t1:dt:t2;
...
%%
%check Hs
HScheck1=4.004*sqrt(abs(trapz(2*pi*freq,S)));
...
coef=mean(Q.\S);
Q=coef*Q;
HScheck2=4.004*sqrt(trapz(2*pi*freq,Q));
SPEC=coef*SPEC;
...
surf(SPEC);
view(2)
set(gca,'ytick',[1:2:length(SPEC(:,1))],'yticklabel',dir0(1:2))
set(gca,'xtick',[1:4:length(SPEC(1,:))],'xticklabel',freq(1:4:end))
xlim([1 length(SPEC(:,1))])
ylim([1 length(SPEC(1,:))])
xlabel('Freq [ Hz ]')
ylabel('Dir [ Deg. ]')
colorbar
